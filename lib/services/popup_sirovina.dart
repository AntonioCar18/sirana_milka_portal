...
class _PopupSirovinaState extends State<PopupSirovina> {
  ...
  Future<bool> updateStatus() async{
    final url = Uri.parse('http://app.sirana-milka.hr:8081/milkaservice/api/add-resource-quantity');
    ...
    if(novaKolicina < 0 || aktualnaKolicina < 0){
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Količina ne može biti negativna.'),
        ),
      );
      return false;
    }
    ...
    if(novaKolicina == 0 && aktualnaKolicina == 0){
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Molimo unesite količinu za ažuriranje.'),
        ),
      );
      return false;
    }
